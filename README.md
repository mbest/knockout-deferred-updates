...
This plugin includes a task scheduler that queues deferred tasks to be processed after the currently running program context is complete. This is used internally for updating computed observables and subscriptions. You can also directly add deferred tasks and alter the task context through the `ko.tasks` interface.
   ...
   * `ko.tasks.processDelayed(evaluator[, distinct])`
      ...
      `processDelayed` adds a function to the task queue. If a function is added more than once within the current task context, its place in the queue is simply moved to the end (unless `distinct` is *false*). `processDelayed` takes two parameters: `evaluator` is the function you want queued; `distinct` (optional; default is *true*), if *false*, queues the function without checking if it’s already queued.
...
1. Computed observables use an `equalityComparer` function to determine whether their value has changed. Unlike regular observables, the default behavior for computed observables is to always consider the value changed. You can modify the behavior for all computed observables by setting `ko.computed.fn.equalityComparer` to a function that compares the two values *(old, new)* and returns *false* if they’re different. You can modify a specific computed observable by setting its `equalityComparer` property or by using the `notify` extender with a value other than `always` (e.g. `ko.computed(...).extend({notify:'notalways'})`).
